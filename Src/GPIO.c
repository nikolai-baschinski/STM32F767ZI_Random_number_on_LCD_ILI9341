...
void init_GPIO(struct GPIO* p_gpio)
{
  ...
  GPIOC->PUPDR &= ~(3 << (13 * 2));
  GPIOC->PUPDR |=  (2 << (13 * 2));   // Pull-down for user button B1
...
uint8_t GPIO_button_pushed()
{
  ...
  if(loc_p_gpio->button_pushed_memory == 0 && loc_p_gpio->button_pushed == 1) {
    loc_p_gpio->debounce_counter = 0;
    rv = 1;
  }
  return rv;
}
...
{
  if(p_pi->cntr_10ms % 100 == 0) {
    GPIO_toggle_green_LED();
  }

  p_pi->gpio.button_pushed_memory = p_pi->gpio.button_pushed;

  if(GPIOC->IDR & (1 << 13)) {
    p_pi->gpio.debounce_counter++;
      if(p_pi->gpio.debounce_counter >= 5) {
        p_pi->gpio.button_pushed = 1;
    }
  } else {
    p_pi->gpio.button_pushed = 0;
  }
}
